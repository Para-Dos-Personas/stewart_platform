...
COUNTDOWN_SECONDS = 60  # change this to whatever you want
...
def draw_timer(frame, seconds_left, cx, cy):
    # Background circle
    radius = 55
    color = (0, 200, 0) if seconds_left > 10 else (0, 60, 255)
    cv2.circle(frame, (cx - 0, 60), radius, (30, 30, 30), -1)
    cv2.circle(frame, (cx - 0, 60), radius, color, 3)
    # Timer text
    ...
    tx = (cx) - text_size[0] // 2
    ty = 60 + text_size[1] // 2
    cv2.putText(frame, timer_str, (tx, ty),
...
state            = "waiting"
show_reset_flash = 0
timer_start      = None   # time.time() when tracking began / timer last reset
...
while True:
    ...
    # --- stretch frame to fill whatever window size we have ---
    ...
    if state == "tracking" and timer_start is not None:
        ...
        seconds_left = max(0, COUNTDOWN_SECONDS - elapsed)
        draw_timer(frame, seconds_left, center_x, center_y)
        ...
        if seconds_left <= 0:
            ...
            last_sent_pitch = 0
            last_sent_roll  = 0
            print("Timer hit 0 — maze auto-reset!")
    # ─────────────────────────────────────────────────────────
    ...
    if results.multi_hand_landmarks and results.multi_handedness:
        for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
            ...
            if state == "tracking":
                ...
                cv2.putText(frame, f"Offset X: {offset_x:.1f}", (10, 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
                cv2.putText(frame, f"Offset Y: {offset_y:.1f}", (10, 60),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                cv2.putText(frame, f"Pitch: {rel_pitch:.2f}", (10, 90),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                cv2.putText(frame, f"Roll:  {rel_roll:.2f}",  (10, 120),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
                cv2.putText(frame, f"Mapped Pitch: {mapped_pitch:.2f}", (10, 150),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (100, 255, 100), 2)
                cv2.putText(frame, f"Mapped Roll:  {mapped_roll:.2f}",  (10, 180),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (100, 255, 100), 2)
        ...
        # Hand lost — full reset back to waiting
        if state == "tracking":
            print("Hand lost — resetting maze and returning to waiting.")
            reset_maze()
            show_reset_flash = 45
            state       = "waiting"
            timer_start = None
        ...
        cv2.putText(frame, "No hand detected — show hand at center",
                    (center_x - 200, center_y + box_size + 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (180, 180, 180), 2)
    ...
    if key == ord('q'):
        ...
        reset_maze()
        show_reset_flash = 45
        timer_start      = time.time() if state == "tracking" else None
        pitch_buffer.clear()
        roll_buffer.clear()
        last_sent_pitch = 0
        last_sent_roll  = 0
        print("Manual reset — maze flat, timer restarted!")
